...
#' @examples
#' \dontrun{
#' tlm_data <- read.payload("NS57_parsedPackets.txt", "LINK-TLM")
#' irene_data <- read.payload("NS57LaunchData.txt", "IRENE")
#' }
             ...
             timezone = Sys.timezone())
    {
        if (data_source %in% c("LINK-TLM", "CellTracker", "IRENE"))
        {
            if (data_source == "LINK-TLM")
            {
                ...
                {
                    launch_data <-
                        tail(subset(parsed_data, subset = parsed_data$DateTime <= start_time),
                             n = 1)
                }
                ...
                {
                    landing_data <-
                        head(subset(parsed_data, subset = parsed_data$DateTime >= end_time),
                             n = 1)
                }
                ...
                {
                    source_log <- subset(parsed_data, subset = parsed_data$Callsign == source_name)
            ...
            else if (data_source == "IRENE")
            {
                ...
                # get Unix epoch timestamps (IRENE records in Zulu time in American date format to separate Date and Time columns)
                parsed_data$DateTime <-
                    as.POSIXct(
...
.distances <- function(lat_1, lon_1, lat_2, lon_2)
{
    R <- 6378137
    a <-
        sin(((lat_2 - lat_1) * pi / 180) / 2) ^ 2 + cos(lat_1 * pi / 180) * cos(lat_2 * pi / 180) * sin(((lon_2 - lon_1) * pi / 180) / 2) ^ 2
...
{
    output <-
        c(0, (positions[1:(length(positions) - 1)] - positions[2:length(positions)]) / as.numeric(times[1:(length(positions) - 1)] - times[2:length(times)], units = "secs"))
